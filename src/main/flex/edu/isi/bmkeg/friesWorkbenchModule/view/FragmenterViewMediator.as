package edu.isi.bmkeg.friesWorkbenchModule.view
{
	...
	public class FragmenterViewMediator extends Mediator
	{
		...
		override public function onRegister():void {
			...
			this.dispatch( new ListOntologyEvent(oQo, true) );
		...
		private function updateFragments(event:ListFTDFragmentResultEvent):void {
			...
			// Scroll to the first fragment.
			var firstFragment:FTDFragmentBlock = null;
			for (var pStr:* in holder ) {
				...
				if( firstFragment == null && fh.fragmentsAdded.length > 0) {
					firstFragment = FTDFragmentBlock(fh.fragmentsAdded.getItemAt(0));
				}
			}			
			if( firstFragment != null )
				view.pgList.ensureIndexIsVisible(firstFragment.p-1);
		...
		private function updateOntologies(event:ListOntologyResultEvent):void {
			...
			for each( var lvi:LightViewInstance in this.view.ontologies ) {
				if(lvi.vpdmfLabel == "bigMechInt") {					
					this.view.ontologyCombo.selectedItem = lvi;
					break;
				}
			}
			
			this.view.ontologyCombo.enabled = false;
			this.view.autoComplete.visible = false;
			this.view.dumpToBratButton.visible = false;
